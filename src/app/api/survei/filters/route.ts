...
export async function GET() {
  try {
    ...
    const [fungsiRows] = await dbConnection.execute(`
      SELECT DISTINCT fungsi FROM survei ORDER BY fungsi
    `);

    // Get unique values for periode
    const [periodeRows] = await dbConnection.execute(`
    ...
    const fungsiOptions = (fungsiRows as any[]).map(row => ({
      name: row.fungsi,
      uid: row.fungsi
    }));

    const periodeOptions = (periodeRows as any[]).map(row => ({
      name: row.periode,
      uid: row.periode
    }));

    const tahunOptions = (tahunRows as any[]).map(row => ({
